...
class OpenAICompatibleComicProvider:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {'provider': self.name, 'enabled': True, 'configured': configured,
                ...
                'supportsVision': True,
                'reason': None if configured else 'COMIC_AI_BASE_URL and COMIC_AI_MODEL are required'}
    ...
    def analyze_comic_pages(self, pages: list[dict[str, Any]], context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        return self._request('Analyze comic pages with evidence and uncertainty. Use the requested semantic schema.', {'pages': safe_pages, 'previousContext': context or {}}, images)

    def adapt_story(self, analysis: dict[str, Any], settings: dict[str, Any]) -> dict[str, Any]:
        return self._request('Adapt this source analysis into a child-safe story. Preserve evidence mappings and label new dialogue as adapted.', {'analysis': analysis, 'settings': settings})

    def generate_episode(self, story: dict[str, Any], settings: dict[str, Any]) -> dict[str, Any]:
        return self._request('Generate a production Episode JSON with grounded image/video prompts and stable character speaker keys.', {'adaptedStory': story, 'settings': settings})
