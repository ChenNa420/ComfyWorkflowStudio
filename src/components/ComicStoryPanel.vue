...
function chooseIssue(item:Issue){
  storyWatchNonce++;collectingGpt.value=false
...
async function watchGptStory(taskId:string){
  const nonce=++storyWatchNonce
  collectingGpt.value=true
  const deadline=Date.now()+10*60*1000
  while(nonce===storyWatchNonce&&Date.now()<deadline&&activeTask.value?.id===taskId&&!gptResult.value){
    try{
      if(await collectGptStoryOnce(taskId)){
        collectingGpt.value=false
        return
      }
    }catch(value){
      if(nonce!==storyWatchNonce)return
      collectingGpt.value=false
      error.value=value instanceof Error?value.message:'自动获取 GPT 结果失败'
      return
    }
    await sleep(3500)
  }
  if(nonce===storyWatchNonce&&!gptResult.value){
    collectingGpt.value=false
    notice.value='暂未检测到新的 GPT 完整回复。完成生成后可点击“立即获取 GPT 结果”重试。'
  }
}
...
async function prepareGptDraft(){
  ...
  try{
    ...
    const value=await postJson(`/api/gpt-image/tasks/${encodeURIComponent(taskId)}/prepare-story`,{
      prompt:directorPrompt.value,
      gptUrl:url,
    })
    notice.value=`已准备完成：${value.attachmentCount||activeTask.value.source.selectedPages.length} 张漫画页和任务说明已放入童语工坊。请检查后手动点击“发送”；工作台会自动等待并回传结果。`
    void watchGptStory(taskId)
  }catch(value){
...
onUnmounted(()=>{storyWatchNonce++;collectingGpt.value=false;lifecycle.abort()})
</script>
  ...
  <div class="director-shell">
    <section class="director-hero panel">
      <div>
        ...
        <p>选择 1–12 个 PDF/漫画页面，创建手动 GPT Task。点击“准备到童语工坊 GPT”后，系统自动上传页面并填好任务说明；你只负责检查后点击发送，工作台会自动读取、修复、校验并回传故事结果。</p>
      </div>
      <div class="bridge-pill ok">
        <Sparkles :size="16"/>
        <div>
          <b>手动模式</b>
          <small>自动准备 → 你手动发送 → 自动回传</small>
    ...
    <template v-else>
      ...
      <section class="panel result-stage">
        <div class="section-title"><div><span class="step-number">4</span><div><h3>{{gptResult?'GPT 已返回故事与分镜':'等待 GPT 自动回传'}}</h3><p>{{gptResult?'结果已自动写入工作台，可以继续按 Shot 手动生成关键帧。':collectingGpt?'正在监听童语工坊的新回复；检测到完整结果后会自动校验并进入 Step 4。':'发送完成后，工作台会自动读取 GPT 回复；也可点击“立即获取 GPT 结果”。'}}</p></div></div><span v-if="gptResult" class="ready-pill">{{gptResult.shots.length}} Shots · READY</span></div>
        ...
        <template v-else>
          <div class="waiting-flow"><div class="done"><span>✓</span><b>Task 已创建</b><small>{{activeTask?'准备源页与提示词':'先创建 Task'}}</small></div><i></i><div :class="{active:!!activeTask}"><span>2</span><b>你手动发送</b><small>检查图片与任务说明</small></div><i></i><div :class="{active:collectingGpt}"><span>3</span><b>自动取回 JSON</b><small>{{collectingGpt?'正在监听 GPT 回复':'等待新回复'}}</small></div><i></i><div><span>4</span><b>自动写回</b><small>校验后进入动画生产</small></div></div>
          <details class="manual-import"><summary>应急备用：手动粘贴 GPT JSON</summary><p>正常情况下无需复制 JSON。仅当自动回传不可用时，在这里粘贴完整结果；工作台会继续执行相同校验。</p><textarea v-model="manualJson" rows="10" placeholder='{"sourceUnderstanding":{},"creativeStory":{"title":"..."},"characterDefinitions":[],"sceneDefinitions":[],"shots":[...]}'></textarea><button class="secondary" :disabled="!activeTask||!manualJson.trim()" @click="importManualResult"><Braces :size="15"/>校验并导入备用 JSON</button></details>
        </template>
      </section>
    </template>
  </div>
