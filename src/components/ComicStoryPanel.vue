...
type Shot = { id:number; title:string; sourcePage:number; duration:number; sourceText?:string; english:string; chinese:string; imagePrompt:string; videoPrompt:string; negativePrompt:string; sourcePages?:number[]; dialogueSource?:string }
...
const story = ref({ title:'', style:'温馨治愈', audience:'3-8岁儿童', language:'中文（简体）', shotCount:6, aspectRatio:'9:16', level:'Pre-A1', duration:30, fidelity:'balanced', adaptationStrength:'medium', educationalGoal:'', preserveCharacterNames:true, preserveDialogues:true, autoShotCount:true })
...
async function loadSuggestedRoots(){
  try{
    ...
    if(response.ok){const value=await response.json(); suggestedRoots.value=value.roots||[]; if(!rootPath.value && suggestedRoots.value.length){rootPath.value=suggestedRoots.value[0]; await scanRoot()}}
    const status=await fetch('/api/comic-story/ai/status'); if(status.ok) provider.value=await status.json()
  }catch{}
}
...
async function semanticAnalyze(){
  if(!selectedIssue.value || !providerReady.value || !canAnalyze.value) return
  loading.value='semantic'; error.value=''
  const pages=Array.from({length:endPage.value-startPage.value+1},(_,index)=>startPage.value+index)
  const stages=pages.flatMap(page=>[`Analyzing page ${page}/${endPage.value} · Dialogue`,`Analyzing page ${page}/${endPage.value} · Visual`]).concat(['Resolving characters','Building story summary'])
  let progressIndex=0; analysisProgress.value=stages[0]
  progressTimer=window.setInterval(()=>{progressIndex=Math.min(progressIndex+1,stages.length-1);analysisProgress.value=stages[progressIndex]},3500)
  try{ semantic.value=await postJson('/api/comic-story/semantic-analyze',{token:selectedIssue.value.token,startPage:startPage.value,endPage:endPage.value,forceRefresh:false}) }
  catch(value){error.value=value instanceof Error?value.message:'AI 语义分析失败'}finally{if(progressTimer)window.clearInterval(progressTimer);progressTimer=undefined;analysisProgress.value='';loading.value=''}
}

async function adaptStory(){
  if(!semantic.value) return
  loading.value='adapt'; error.value=''
  try{const value=await postJson('/api/comic-story/adapt-story',{semanticAnalysisId:semantic.value.id,style:story.value.style,audience:story.value.audience,language:story.value.language,level:story.value.level,educationGoals:story.value.educationalGoal?[story.value.educationalGoal]:[],fidelity:story.value.fidelity,preserveCharacterNames:story.value.preserveCharacterNames,preserveCorePlot:true,preserveDialogue:story.value.preserveDialogues,shotCount:story.value.shotCount,autoShotCount:story.value.autoShotCount,aspectRatio:story.value.aspectRatio});adapted.value=value.adaptedStory}
  catch(value){error.value=value instanceof Error?value.message:'故事改编失败'}finally{loading.value=''}
}
...
async function generateAiEpisode(){
  ...
  try{
    const resolvedShotCount=story.value.autoShotCount?(adapted.value?.recommendedShotCount||story.value.shotCount):story.value.shotCount
    const value=await postJson('/api/comic-story/generate-episode',{adaptedStory:adapted.value,settings:{source:semantic.value.source,level:story.value.level,age:story.value.audience,duration:story.value.duration,aspectRatio:story.value.aspectRatio,shotCount:resolvedShotCount,autoShotCount:story.value.autoShotCount}})
...
onUnmounted(()=>{if(progressTimer)window.clearInterval(progressTimer)})
</script>
    ...
    <template v-else-if="step===2 && selectedIssue">
      <section class="comic-analysis-grid">
        ...
        <article class="panel comic-stage analysis-panel">
          <span class="eyebrow">SOURCE ANALYSIS</span><h3>解析与分析漫画</h3><p>先完成只读本地提取，再按页分别识别对白与视觉语义，最后在本地完成角色解析、Evidence 和故事融合。</p>
          ...
          <div v-if="analysisProgress" class="analysis-progress"><LoaderCircle class="spin" :size="14"/><b>{{analysisProgress}}</b><small>Pagewise Fusion 会保留已经完成的页面阶段。</small></div>
          ...
          <section v-if="semantic" class="quality-review"><span class="eyebrow">AI QUALITY REVIEW</span><h3>AI 质量审查</h3><div v-if="quality" class="quality-metrics"><span>页 {{quality.selectedPages?.join(', ')}}</span><span>角色 {{quality.charactersDetected}}（合并前 {{quality.charactersBeforeMerge}} / 后 {{quality.charactersAfterMerge}}）</span><span>需审查 {{quality.charactersNeedingReview}}</span><span>对白 {{quality.dialoguesDetected}}</span><span>Speaker {{quality.speakerResolved}} / Unknown {{quality.speakerUnknown}}</span><span>场景 {{quality.scenesDetected}}</span><span>事件 {{quality.plotEventsDetected}}</span><span>Evidence {{quality.evidenceCount}} / Invalid {{quality.evidenceInvalid}}</span><span>Gate {{quality.qualityGatePassed?'PASS':'REVIEW'}}</span><span>Warnings {{quality.semanticWarnings}}</span></div><div v-if="semantic.pageMetrics?.length" class="page-metrics"><b v-for="item in semantic.pageMetrics" :key="item.page">P{{item.page}} · Dialogue {{item.dialogueCount}} · Character {{item.characterCount}} · Scene {{item.sceneCount}} · Event {{item.eventCount}}</b></div><h4>Character Identity</h4><article v-for="item in semantic.characters" :key="item.id"><b>{{item.id}} · {{item.name}}</b><span :class="['confidence-badge',item.confidence>=.75?'high':item.confidence>=.45?'medium':'low']">{{item.confidence>=.75?'High':item.confidence>=.45?'Medium':'Low'}}</span><p>{{item.appearance}} · {{item.clothing}}</p><div class="evidence-chips"><button v-for="page in item.pages" :key="page" @click="currentPage=page">P{{page}}</button></div></article><h4>Dialogue & Evidence</h4><article v-for="(item,index) in semantic.dialogues" :key="index"><b>P{{item.page}} · {{item.speakerId||'Unknown'}}</b><span :class="['confidence-badge',item.confidence>=.75?'high':item.confidence>=.45?'medium':'low']">{{item.confidence>=.75?'High':item.confidence>=.45?'Medium':'Low'}}</span><p>{{item.text}}</p><button class="evidence-link" @click="currentPage=item.page">查看 Evidence P{{item.page}}</button></article><div v-if="semantic.warnings?.length" class="quality-warnings"><b>Warnings</b><p v-for="warning in semantic.warnings" :key="warning">{{warning}}</p></div></section>
...
.analysis-progress{display:grid;grid-template-columns:auto 1fr;gap:3px 8px;align-items:center;margin-top:8px;padding:9px;border-radius:8px;background:#f4f2ff;color:#5f53d1;font-size:9px}.analysis-progress small{grid-column:2;color:#7f88a3}.page-metrics{display:grid;gap:4px;margin-top:8px}.page-metrics b{padding:6px 8px;border-radius:7px;background:#f7f8fc;color:#65708d;font-size:8px}
